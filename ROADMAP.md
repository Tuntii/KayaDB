...
**Last updated:** 2026-09-05 (v0.2.0 cut)
...
7. **M22 — Rebalancing, merges & placement ✅ production path** — Complete (2026-07-17): shared-engine `merge_with_next` + wire `MERGE_RANGE` (17) + `kayactl range merge` (no physical key move; orphan group reclaim **shipped, #30**); admin `TRANSFER_LEADER` (18) (step-down; no TimeoutNow); learner membership flag + `PROMOTE_LEARNER` (19) (learners receive log, do not vote/campaign); **advisory** `REBALANCE_PLAN` (20) range-count heuristic (live migrate landed later as `MOVE_RANGE` (21), #24); drain mode (`--drain` / `KAYA_DRAIN`) + decommission runbook; Dashboard v1 read-only HTTP (`--dashboard-addr`: `/health`, `/v1/ranges`, `/v1/raft`). Spec: `spec/docs/range-routing-spec.md`. *Not in this path:* locality tags, auto size-threshold split, TimeoutNow preferred-candidate election, full chaos add/drain/decommission gate.
8. **M23 — Cross-shard transactions ✅ production path** — Complete (2026-07-17): shared-engine 2PC records (`\x00txn/rec|intent/…`, `apply_txn_{prepare,commit_2pc,abort_2pc}`); RaftCommand types 5/6/7 (`TxnPrepare` / `TxnCommit2pc` / `TxnAbort2pc`); server `txn_coord::commit_cross_group` when `TXN_COMMIT` spans >1 group via `StaticRangeTable` (client-transparent); **sequential** prepare then commit/abort proposes (not parallel-commit stretch); startup recovery (`Preparing`/`Prepared` → abort; `Committing` → finish); TLA+ sketch `spec/specs/txn/TwoPhaseCommit.tla`; IT `test_cross_range_txn_commit` + multi-range bank `test_multi_range_bank_sum_invariant`. Spec: `spec/docs/transactions-spec.md` §17. *Not in this path:* multi-range Jepsen bank under full split+merge+rebalance+kill+partition chaos matrix. Parallel prepare/commit, the durable global decision log and cross-node participant forwarding shipped in **#26** (see the residual-risk table). HLC uncertainty-interval wait/clamp shipped in **#27**.
...
|---|---|
| No physical key migration on migrate | **Live `MOVE_RANGE` (21) shipped (#24):** atomic routing cutover via one committed `RangeMeta` entry, `kayactl range move`, client API, sim crash property, IT under load, `bank-mr-move` chaos gate. Residual: no key copy — every group in a process shares one engine, so a move changes placement, not bytes; snapshot+delta+dual-write is spec'd in range-routing §3d for per-group engines. Cross-group read freshness at cutover still relies on the shared engine when the group-0 leader does not lead the source group |
...
| Cross-group 2PC coordination | **Shipped (#26):** parallel prepare/commit/abort under a per-phase timeout; durable global decision log (`TxnDecision` type 9 → `\x00txn/dec/…` on group 0) written before any participant commits; recovery follows it, and a participant never aborts an acked `Prepared` record on its own (TXN-2PC-8) — in-doubt records are held and released by the decision, or by the abort decision a new meta-group leader records for orphans; `TXN_FORWARD` (22) forwards to a participant group's own leader, so one node no longer has to lead every participant group. *Residual:* forwarding is plaintext-only (fails, and the txn aborts, on a TLS-enabled cluster), leader lookup is single-attempt, decision records are never GC'd, and a txn stays in doubt (invisible, intents held) from coordinator death until the next meta-group election |
| HLC uncertainty-interval wait/clamp | **Shipped (#27):** `max_clock_offset_micros` bound; `Engine::sync_clock` rejects a too-far-ahead remote sample (`checked_update`); `prepare_hlc_write_sequence` waits out an in-bound lead before exposing commit_ts. Residual: cross-node `sync_clock` wiring (no wire caller yet) and a read-side uncertainty retry are follow-on |
...
| Linearizability minimal counterexample | **Shipped (#34):** WGL `minimal_counterexample` + MUS enumeration (`minimal_unsatisfiable_subsets`) + `kaya-wgl` JSONL explorer; Jepsen violation report still includes the greedy shrink |
| Full multi-tenancy | **First tenant isolation shipped (#29):** named tenants with exclusive key prefixes (`--tenant-file` / `KAYA_TENANT_FILE`), hard deny on cross-tenant keyed ops, tenant id on audit JSONL. PrefixAcl still available independently (AND when both set). Residual: resource quotas, RBAC, billing |
...
- Linearizability checker'in production'a yakın versiyonu (WGL) — ✅ M11 (`LinearizabilityChecker::check_concurrent`, WGL algorithm, `Vec<String>` violation report; used in Jepsen full gate). Daha zengin raporlama (minimal counterexample + MUS enumeration + `kaya-wgl` explorer) ✅ #34
