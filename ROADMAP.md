...
5. **M20 — Multi-raft foundation ✅ foundation** — Complete (2026-07-12): Envelope.group_id wire multiplexing; per-group paths (data_dir/groups/{id}/ for id!=0, legacy root for group 0); MultiRaftHost + StaticRangeTable + GroupId; coalesced 	ick_all; kaya_core::Hlc (pack physical<<16|logical); spec spec/docs/multi-raft-spec.md. *Exit met (foundation):* unit tests for 2 independent groups + HLC monotonicity + codec group_id. *Limitations (follow-on):* ClusterNode still single group 0; no client key routing; no dynamic splits; no per-range Jepsen; OTel group_id attribute stub only (no full trace-context propagation); no live clock-skew nemesis; HLC not yet wired as engine commit_ts source.
