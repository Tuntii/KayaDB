...
| Cross-group 2PC coordination | **Shipped (#26):** parallel prepare/commit/abort under a per-phase timeout; durable global decision log (`TxnDecision` type 9 → `\x00txn/dec/…` on group 0) written before any participant commits; recovery resolves `Preparing`/`Prepared` against it; `TXN_FORWARD` (22) forwards to a participant group's own leader, so one node no longer has to lead every participant group. *Residual:* forwarding is plaintext-only (fails, and the txn aborts, on a TLS-enabled cluster), leader lookup is single-attempt, decision records are never GC'd, and a node whose group-0 log lags at open still fail-closes that record |
