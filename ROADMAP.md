...
10. **M25 — Scale proof & ecosystem close-out 🟡** — Go txn/retry + TypeScript client + conformance v3 landed; **perf envelope v2** (multi-key SI + multi-range 2PC smoke budgets in `kaya-bench`/`BENCHMARKS.md`) + **deployment guide v2** (`docs/deployment-guide-v2.md`, M22–M24 flags + range ops) + SLO notes. Remaining: ⬅ *scheduled profiling CI*, Jepsen grand matrix, ⬅ *linearizability minimal counterexample*, Zig client (optional). *Exit:* north-star yeniden değerlendirilir → **v0.2.0**.
