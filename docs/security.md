...
**Keyring.** A keyring is a small text file (`active <id>` + one or more `key <id> <64-hex-char>` lines) holding the active key plus every previous-generation key still needed to decrypt old files. It supersedes the single raw-bytes `--encryption-key-file` for deployments that want rotation; the two flags are mutually exclusive on `kayadb-server`.
