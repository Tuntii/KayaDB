use std::sync::Arc;
use kaya_core::{EngineConfig, DurabilityMode};
use kaya_io::FileDisk;
use kaya_engine::{Engine, WriteOptions, ReadOptions};
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    engine.put(b"key1".to_vec(), b"value1".to_vec(), write_opts.clone()).await?;

    println!("Putting key2=value2...");
    engine.put(b"key2".to_vec(), b"value2".to_vec(), write_opts.clone()).await?;
    ...
    if let Some(val) = engine.get(b"key2", ReadOptions::default()).await? {
        println!("Durable retrieval key2: {}", String::from_utf8(val.to_vec())?);
    }
