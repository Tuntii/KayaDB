...
- **Dashboard v2 Phase A (#31):** read-only HTTP JSON on `--dashboard-addr` gains `GET /v1/cluster` (node_id, drain, range_count, leader_group_ids, meta_epoch), `GET /v1/leadership` (group_id → leader/term/role), per-range `healthy` on `GET /v1/ranges` (true when the hosting group has a known leader), and `GET /v1/errors` (in-memory ring, cap 50; dashboard 404s plus client `STATUS_ERROR` / auth deny). `GET /health` stays `{"ok":true}`. Runbook `docs/runbooks/dashboard.md`. **Phase B** (eBPF/fsync attribution) and **Phase C** (profiling CI) deferred: they need a Linux perf/capability runner.
- **TypeScript client TXN + RetryPolicy (#32):** `clients/kaya-ts` now has Snapshot Isolation `beginTxn` / `Transaction` get-put-delete-commit-rollback (opcodes 9–12, `STATUS_TXN_CONFLICT=3`, `CLIENT\x00` token framing on TXN ops), `RetryPolicy` matching Go/Rust defaults (4 attempts, 50ms base, 2s cap, jitter, 5s timeout; leader redirects do not consume an attempt), opcode coverage documented in `clients/kaya-ts/README.md`, and a CI `kaya-ts` job running `npm test`. Zig client remains **wontfix/deferred** for this train (no skeleton)
- **Parallel 2PC + durable global decision log (#26):** cross-group `TXN_COMMIT` now fans `TxnPrepare` / `TxnCommit2pc` / `TxnAbort2pc` out to all participants **in parallel** under a per-phase timeout (`DEFAULT_PHASE_TIMEOUT`, 5s); new `RaftCommand::TxnDecision` (type 9) writes `\x00txn/dec/{txn_id}` on the **meta group (group 0)** before any participant is asked to commit, and an explicit abort decision on the failure path; `Engine::apply_txn_decision` / `read_txn_decision`; open-time recovery resolves records against the decision log, and a participant no longer aborts an **acked** prepare on its own (**TXN-2PC-8**): with no local decision, `Preparing` (never acked) aborts but `Prepared` is held in doubt with its intents — aborting it could produce a partial user-visible commit when this node's group-0 log lagged. Applying a `TxnDecision` resolves any matching local record, and a node gaining meta-group leadership records an abort decision for orphaned prepares (`txn2pc_pending` on `RecoveryReport`); new internal opcode 22 `TXN_FORWARD` (21 is `MOVE_RANGE`) lets the coordinator forward a 2PC command to a participant group's own leader over the client RPC, so cross-group commits no longer require one node to lead every participant group; forwarding is operator-token gated and refused when a client token / ACL is configured without one. Spec: `transactions-spec.md` §17 (coordinator role, decision-log schema, coordinator-death recovery, TXN-2PC-6/7)
- **Live range migrate `MOVE_RANGE` (#24):** opcode **21** (admin; operator token via `ADMIN\x00` framing) reassigns one range to a target Raft group under load. `StaticRangeTable::move_range` keeps `range_id` + bounds and bumps the range `epoch`, `meta_epoch` and `next_group_id`; cutover is a single committed `RaftCommand::RangeMeta` on group 0 (CAS on `base_epoch`, same persist + host path as split/merge), ordered *host target → quiesce source → commit*. `kayactl range move <start> <target-group>`, `KayaClient::move_range` + `set_operator_token`. Keys are **not** copied — every group in a process shares one engine — and the snapshot+delta+dual-write protocol for per-group engines is spec'd in `spec/docs/range-routing-spec.md` §3d with failure modes and invariants RANGE-INV-01..05. Sim property `move_range_no_lost_or_duplicated_keys_across_crash` (crash at every point mid-migrate), IT `test_range_move_under_concurrent_load`, Jepsen `bank-mr-move` chaos gate (MOVE_RANGE + kill, documented subset of the nightly matrix), runbook `docs/runbooks/move-range.md`
- **Orphan Raft group reclaim after merge (#30):** `MultiRaftHost::remove`; `cluster::replication::reclaim_orphan_groups` runs on every drain pass, unhosts + `remove_dir_all`s the data dir of any hosted group no longer referenced by the range table (never group 0, only once `commit_index == last_applied`); idempotent/crash-safe since startup only rehosts groups the persisted range table still references; `kaya_range_orphan_groups` gauge + `kaya_range_orphan_groups_reclaimed_total` counter on `/metrics`; spec §3e; IT `test_range_merge_reclaims_orphan_group`
- **HLC uncertainty-interval wait/clamp (#27):** `EngineConfig.max_clock_offset_micros` (default 500ms) / `ClusterConfig.max_clock_offset_micros` / CLI `--max-clock-offset-micros` / env `KAYA_MAX_CLOCK_OFFSET_MICROS`; `Hlc::checked_update` rejects (`KayaError::ClockSkew`) a remote HLC sample more than the bound ahead of local wall-clock time (`Engine::sync_clock` now returns `Result`); `prepare_hlc_write_sequence` waits out an in-bound clock lead (`Hlc::lead_over_wall_ms`) before a write's commit_ts is durable/exposed. Spec `spec/docs/transactions-spec.md` §17.7, `spec/docs/multi-raft-spec.md` §8; operator runbook `docs/runbooks/hlc-clock-skew.md`; unit tests with injected clock skew in `kaya-core::hlc` and `kaya-engine`
- **Online encryption key rotation (#28):** `Keyring` (active + previous AES-256 keys by id) in `kaya-io`; envelope gains a `key_id` (new `KAYAENC2` format, AAD-bound) while pre-existing `KAYAENC1` files stay byte-for-byte unchanged and are treated as key id 0; read path decrypts by the id in each file's header, write path always reseals under the active key (old files upgrade lazily on next write — no background re-encrypt, documented online dual-key read window); `kayadb-server --encryption-keyring-file` / `KAYA_ENCRYPTION_KEYRING_FILE` (mutually exclusive with `--encryption-key-file`); `kayactl encryption init|rotate|list|verify`; `docs/security.md` §7.1 spec + `docs/runbooks/key-rotation.md`
- **Durable range meta (#25):** `StaticRangeTable` encode/decode preserves `meta_epoch` / id counters; `RaftCommand::RangeMeta` (type 8) on group 0 with CAS on `base_epoch`; SPLIT/MERGE commit via that entry; `{data_dir}/range-table.bin` + startup restore; Raft snapshot **v2** embeds the table; optional `MEPO` client epoch on PUT/GET/DELETE/SCAN returns `RANGE_MOVED` when stale; ITs `test_range_split_survives_restart`, `test_range_split_survives_all_nodes_restart`, `test_stale_meta_epoch_returns_range_moved`; sim crash + snapshot catch-up
