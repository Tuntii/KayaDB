...
### Added
- **Orphan Raft group reclaim after merge (#30):** `MultiRaftHost::remove`; `cluster::replication::reclaim_orphan_groups` runs on every drain pass, unhosts + `remove_dir_all`s the data dir of any hosted group no longer referenced by the range table (never group 0, only once `commit_index == last_applied`); idempotent/crash-safe since startup only rehosts groups the persisted range table still references; `kaya_range_orphan_groups` gauge + `kaya_range_orphan_groups_reclaimed_total` counter on `/metrics`; spec §3d; IT `test_range_merge_reclaims_orphan_group`
