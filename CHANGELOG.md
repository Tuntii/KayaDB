...
### Added
- **Parallel 2PC + durable global decision log (#26):** cross-group `TXN_COMMIT` now fans `TxnPrepare` / `TxnCommit2pc` / `TxnAbort2pc` out to all participants **in parallel** under a per-phase timeout (`DEFAULT_PHASE_TIMEOUT`, 5s); new `RaftCommand::TxnDecision` (type 9) writes `\x00txn/dec/{txn_id}` on the **meta group (group 0)** before any participant is asked to commit, and an explicit abort decision on the failure path; `Engine::apply_txn_decision` / `read_txn_decision`; open-time recovery resolves `Preparing`/`Prepared` against the decision log (commit decision → finish commit, abort/absent → fail-closed abort) instead of always aborting; new internal opcode 22 `TXN_FORWARD` lets the coordinator forward a 2PC command to a participant group's own leader over the client RPC, so cross-group commits no longer require one node to lead every participant group; forwarding is operator-token gated and refused when a client token / ACL is configured without one. Spec: `transactions-spec.md` §17 (coordinator role, decision-log schema, coordinator-death recovery, TXN-2PC-6/7)
