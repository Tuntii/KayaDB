...
if ($IsLinux -or (Test-Path "/proc/version")) {
    ...
    cargo test -p kaya-ebpf --features kernel-probes bpf_object_loads_without_cap_bpf kernel_load_object_and_drain_injected_ringbuf -- --test-threads=1 2>&1 | Tee-Object -FilePath "$Scratch\kernel-ringbuf-tier-b.log"
